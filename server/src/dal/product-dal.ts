...
// async function addProduct(product: ProductModel) {
//     let sql = `SET IDENTITY_INSERT [market].[product] ON;
// INSERT INTO [market].[product] ( [name], [category_id], [price], [image_url])
// VALUES 
// ( ?, ?, ?, ?)`;
//     let parameters = [
//         product.name,
...
async function addProduct(product: ProductModel): Promise<void> {
    let sql = `
    INSERT INTO market.product (name, category_id, unit_price, img)
    VALUES (@name, @category_id, @unit_price, @img)`;
    
    let parameters = [
        { name: 'name', value: product.name },
        { name: 'category_id', value: product.categoryID },
        { name: 'unit_price', value: product.price },
